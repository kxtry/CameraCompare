...
public class Camera2Helper {
    ...
    private class OnImageAvailableListenerImpl implements ImageReader.OnImageAvailableListener {
        ...
        private byte[] YUV_420_888toNV21(Image image) {
            ...
            if(outputData.isEmpty()) {
                outputData = String.format("ImageFormat:0x%x", image.getFormat());
                outputData += String.format("Image:(%d,%d),YSize:%d,vSize:%d,uSize:%d", image.getWidth(), image.getHeight(), ySize, vSize, uSize);
                outputData += String.format("StridePixel:y(%d,%d) - u:(%d,%d) - v:(%d,%d)", y.getRowStride(), y.getPixelStride(), u.getRowStride(), u.getPixelStride(), v.getRowStride(), v.getPixelStride());
                Log.e(TAG, "Camera2.ImageOutputInfo:" + outputData);
            }
            //U and V are swapped
